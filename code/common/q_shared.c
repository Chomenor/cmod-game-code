...
===================
*/
static void Info_RemoveKey_Size( char *s, const char *key, unsigned int bufSize ) {
	char	*start;
	char	pkey[BIG_INFO_STRING];
	char	value[BIG_INFO_STRING];
	char	*o;

	if ( bufSize > BIG_INFO_STRING ) {
		Com_Error( ERR_DROP, "Info_RemoveKey_Size: oversize bufSize" );
	}

	if ( strlen( s ) >= bufSize ) {
		Com_Error( ERR_DROP, "Info_RemoveKey_Size: oversize infostring" );
	}
...
void Info_SetValueForKey_Size( char *s, const char *key, const char *value, int bufSize ) {
	...
	if ( bufSize > BIG_INFO_STRING ) {
		Com_Error( ERR_DROP, "Info_SetValueForKey_Size: oversize bufSize" );
	}

	if ( strlen( s ) >= bufSize ) {
		Com_Error( ERR_DROP, "Info_SetValueForKey_Size: oversize infostring" );
	}
	...
	Info_RemoveKey_Size (s, key, bufSize);
	if (!value || !strlen(value))
		return;

	Com_sprintf (newi, sizeof(newi), "\\%s\\%s", key, value);

	if (strlen(newi) + strlen(s) >= bufSize)
	{
