...
//
#include "bots/chars.h"				//characteristics
#include "bots/inv.h"				//indexes into the inventory
#include "bots/syn.h"				//synonyms
#include "bots/match.h"				//string matching types and vars
...
int BotChat_Death(bot_state_t *bs) {
	...
	if (BotNumActivePlayers() <= 1) return qfalse;
	//
	if (bs->lastkilledby >= 0 && bs->lastkilledby < level.maxclients)
...
int BotChat_HitTalking(bot_state_t *bs) {
	...
	//
	if (lasthurt_client < 0 || lasthurt_client >= level.maxclients) return qfalse;
	//
